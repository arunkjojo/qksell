...
import { VEHICLE_TYPE, FUEL_TYPE } from '@common/constants';

export const VehiclesSubCategories: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6 pb-20">
            <div className="max-w-7xl mx-auto">
                <>
                    <PageHeader title="Choose Vehicle Type" showBack={true} onBack={goToBack} />
                    <motion.div
                    ...
                    >
                        {VEHICLE_TYPE?.map((item) => {
                            const selected = subCategories && subCategories?.some((data: { name: string; value: string; }) => data?.name === 'vehicle-type' && data.value === item?.name)
                            return (
                                <SelectedButton
                                    key={item.id}
                                    icon={item.icon}
                                    children={item.name}
                                    onClick={() => handleSelect(item?.name, 'vehicle-type')}
                ...
                <>
                    <PageHeader title="Choose Fuel Type" showBack={false} extraClass='mt-4' />
                    <motion.div
                        className="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
